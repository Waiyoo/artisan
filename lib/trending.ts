 ...
 * - Real activity recorded via ArtistMetric table
 */
export async function calculateTrendingArtists(limit = 10) {
  const now = new Date();
  const sevenDaysAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
  const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);

  // Fetch published artists with their recent metrics and baseline views
  ...
  const scoredArtists = artists.map((artist) => {
    ...
    const recentMetrics = artist.metrics.filter((m) => new Date(m.timestamp) >= sevenDaysAgo);
    const sustainedMetrics = artist.metrics;
    ...
    const ageInDays = (now.getTime() - new Date(artist.createdAt).getTime()) / (1000 * 60 * 60 * 24);
    const recencyDecay = Math.max(0.2, 1 / (1 + ageInDays * 0.05));

    // Defensible composite trending score
    const trendingScore = (recentEngagementScore * 1.5 + sustainedEngagementScore * 0.5 + artist.profileViews * 0.1) * recencyDecay;
  ...
  scoredArtists.sort((a, b) => b.trendingScore - a.trendingScore);
