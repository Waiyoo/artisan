// components/public/investmentCard.tsx
    ...
    >
          ...
          <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
            ...
            {investment.location && (
              <span className="text-xs text-neutral-300 flex items-center gap-1 bg-neutral-950/80 backdrop-blur-md px-2.5 py-1 rounded-md border border-neutral-800">
                <MapPin className="w-3 h-3 text-rose-400" /> {investment.location}
              </span>
            )}
          </div>
        )}
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-3">
        <div>
          ...
          {investment.bio && <p className="text-xs text-neutral-400 mt-1.5 line-clamp-2 leading-relaxed">{investment.bio}</p>}
        </div>

        {investment.tags && investment.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 pt-2 border-t border-neutral-800/60">
            {investment.tags.slice(0, 3).map((t) => (
